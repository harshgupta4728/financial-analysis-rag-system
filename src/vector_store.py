...
class FinancialVectorStore:
    ...
    def __init__(self, embedding_model: str = "all-MiniLM-L6-v2", persist_directory: str = "./chroma_db"):
        ...
        try:
            # This is more robust for Streamlit Cloud's ephemeral filesystem
            self.client = chromadb.PersistentClient(path=self.persist_directory)
            logger.info(f"Initialized ChromaDB at {self.persist_directory}")
        except Exception as e:
            logger.error(f"Failed to initialize ChromaDB PersistentClient: {e}. Falling back to in-memory Client.")
            # Fallback to an in-memory client if persistent client fails
            self.client = chromadb.Client()
    ...
    def add_documents(self, documents: List[Dict[str, Any]], collection_name: str) -> bool:
        """
        Add documents to the specified vector store collection.
        """
        try:
            if not documents:
                logger.warning("No documents provided to add.")
            ...
            # Ensure unique IDs for documents
            ids = [doc.get('id', f"doc_{i}_{datetime.now().timestamp()}") for i, doc in enumerate(documents)]
            texts = [doc.get('content', '') for doc in documents]
            metadatas = []

            for doc in documents:
                meta = doc.get('metadata', {})
                meta['type'] = doc.get('type', collection_name)
    ...
    def search_documents(self, query: str, n_results: int = 5, collection_name: str = "reports") -> List[Dict[str, Any]]:
        """
        Search for documents similar to the query in a specific collection.
        """
        try:
            collection_map = {
                "reports": self.reports_collection,
            ...
            logger.error(f"Error searching documents in {collection_name}: {str(e)}")
            return []

    def search_across_all_collections(self, query: str, n_results: int = 8) -> List[Dict[str, Any]]:
        """
        Search across all relevant collections and combine the results.
        """
        all_results = []
        num_per_collection = max(1, n_results // 3)

        for collection_name in ["reports", "news", "market"]:
            results = self.search_documents(query, n_results=num_per_collection, collection_name=collection_name)
    ...
    def get_collection_stats(self) -> Dict[str, Any]:
        ...
        for name, collection in collection_map.items():
            try:
                count = collection.count()
                stats[name] = {'document_count': count, 'name': name}
            except Exception as e:
                logger.warning(f"Error getting stats for {name}: {str(e)}")
                stats[name] = {'document_count': 0, 'name': name}
        return stats
    ...
    def reset_all_collections(self) -> bool:
        ...
        """
        try:
            for name in ["financial_reports", "financial_news", "market_data"]:
                try:
                    self.client.delete_collection(name)
                except Exception:
